...
def connect_hopsworks():
    """Connect to Hopsworks with fallback"""
    try:
        project = hopsworks.login(
            project=PROJECT_NAME,
            api_key_value=HOPSWORKS_API_KEY
        )
        return project, False  # Success, not offline
    except Exception as e:
        error_str = str(e).lower()
        # Check if it's a DNS/connection error
        if "failed to resolve" in error_str or "name or service not known" in error_str or "getaddrinfo failed" in error_str:
            st.warning("⚠️ Using OFFLINE MODE - Hopsworks connection unavailable")
            return None, True  # Offline mode
        else:
            st.error(f"Error connecting to Hopsworks: {str(e)}")
            return None, True  # Offline mode

@st.cache_data(ttl=3600)
def fetch_historical_data(_project, is_offline=False):
    """Fetch last 24 hours from Hopsworks or local CSV"""
    
    # If offline, use local CSV
    if is_offline or _project is None:
        csv_path = LOCAL_DATA_DIR / "karachi_aqi_raw.csv"
        if csv_path.exists():
            ...
            st.error("❌ No local data found! Please run: python create_local_fallback.py")
            return None
    ...
    try:
        ...
        st.warning(f"Hopsworks fetch failed, using local data: {str(e)}")
        # Fallback to local
        return fetch_historical_data(None, is_offline=True)
...
@st.cache_data(ttl=3600)
def get_model_metrics(_project, model_name, is_offline=False):
    """Get model metrics from Hopsworks Model Registry or local file"""
    ...
    folder_name = model_folders.get(model_name, "catboost")
    
    # If offline, use local metadata
    if is_offline or _project is None:
        metadata_path = LOCAL_DATA_DIR / folder_name / "model_metadata.json"
    ...
    try:
        ...
        if not models or len(models) == 0:
            return get_model_metrics(None, model_name, is_offline=True)
...
@st.cache_resource
def load_model(_project, model_name, is_offline=False):
    """Load model from Hopsworks Model Registry or local file"""
    ...
    # If offline, use local model
    if is_offline or _project is None:
        model_path = LOCAL_DATA_DIR / folder_name / "model.pkl"
        if model_path.exists():
            with open(model_path, 'rb') as f:
                return pickle.load(f)
        else:
            st.error(f"❌ No local {model_name} model found! Please run: python create_local_fallback.py")
            return None
    ...
    try:
        ...
        models = mr.get_models(name="karachi_aqi_predictor")
        if not models or len(models) == 0:
            st.warning("No models in Hopsworks, using local model")
            return load_model(None, model_name, is_offline=True)
        ...
        st.warning(f"Hopsworks model load failed, using local: {str(e)}")
        return load_model(None, model_name, is_offline=True)
...
def main():
    ...
    with st.spinner("🔗 Connecting to Hopsworks..."):
        project, is_offline = connect_hopsworks()
    ...
    if is_offline:
        ...
        <div style="background: linear-gradient(135deg, #FFA500 0%, #FF8C00 100%); 
            ...
            ⚠️ OFFLINE MODE - Using Local Data | Hopsworks Unavailable
        </div>
        """, unsafe_allow_html=True)
    
    # NOW create sidebar with correct info
    with st.sidebar:
        ...
        if is_offline:
            ...
            st.markdown("**📂 Data Source:** Local CSV Files")
            st.markdown("**🤖 Model Source:** Local File System")
        else:
            st.markdown("**🟢 Mode:** Online")
            st.markdown("**📂 Data Source:** Hopsworks Feature Store")
            st.markdown("**🤖 Model Source:** Hopsworks Model Registry")
    ...
    if predict_button:
        with st.spinner("🔄 Fetching data and making predictions..."):
            ...
            historical_df = fetch_historical_data(project, is_offline)
            ...
            # Load model
            model = load_model(project, model_choice, is_offline)
            ...
            metrics = get_model_metrics(project, model_choice, is_offline)
            ...
            with info_col1:
                if is_offline:
                    # OFFLINE MODE - Show local data info
                    st.markdown(f"""
                    <div class="info-box">
                        <h4 style="margin-bottom: 1rem;">📊 Data Source</h4>
                        <p><strong>Mode:</strong> 🟠 Offline (Local Files)</p>
                        <p><strong>Source:</strong> Local CSV File</p>
                        <p><strong>File:</strong> local_data/karachi_aqi_raw.csv</p>
                    ...
                    # ONLINE MODE - Show Hopsworks info
                    st.markdown(f"""
                    <div class="info-box">
                        <h4 style="margin-bottom: 1rem;">📊 Data Source</h4>
                        <p><strong>Mode:</strong> 🟢 Online (Live Data)</p>
                        <p><strong>Source:</strong> Hopsworks Feature Store</p>
            ...
            with info_col2:
                if is_offline:
                    # OFFLINE MODE - Show local model info
                    model_folders = {
                        "CatBoost": "catboost",
                        "XGBoost": "xgboost",
                        "RandomForest": "random_forest"
                    }
                    folder_name = model_folders.get(model_choice, "catboost")
                    
                    st.markdown(f"""
                    <div class="info-box">
                        <h4 style="margin-bottom: 1rem;">🤖 Model Details</h4>
                        <p><strong>Mode:</strong> 🟠 Offline (Local Files)</p>
                        <p><strong>Source:</strong> Local File System</p>
                        <p><strong>Model File:</strong> local_data/{folder_name}/model.pkl</p>
                        ...
                        <p><strong>Training Method:</strong> Pre-trained (static)</p>
                    </div>
                    """, unsafe_allow_html=True)
                else:
                    # ONLINE MODE - Show Hopsworks model info
                    st.markdown(f"""
                    <div class="info-box">
                        <h4 style="margin-bottom: 1rem;">🤖 Model Details</h4>
                        <p><strong>Mode:</strong> 🟢 Online (Live Model)</p>
                        <p><strong>Source:</strong> Hopsworks Model Registry</p>
                        <p><strong>Model Name:</strong> karachi_aqi_predictor</p>
                        <p><strong>Selected Model:</strong> {model_choice}</p>
                        <p><strong>Features Used:</strong> 12 engineered features</p>
                        <p><strong>Prediction Targets:</strong> 3-day ahead (1d, 2d, 3d)</p>
                        <p><strong>Training Method:</strong> Automated daily retraining</p>
                    </div>
                    """, unsafe_allow_html=True)
